...
class Radar(MapGenerator):
    ...
    def animateRadar(self, t, ax, showTrueTrajectories=True, showTrueTrajectoriesMeasurements=True, showRadar=True,
        ...
        if showTrueTrajectories:
            for i, traj in enumerate(self.trajectories):
                ax.plot(traj.X[0], traj.X[1], "-", color=self.traj_colors[i % len(self.traj_colors)],
        ...
        if showRadarClutter:
            ax.plot(self.radarMeasurements[t][0], self.radarMeasurements[t][1], "*", color="orange",
                    label="Radar Clutter")
            for i in range(len(self.radarMeasurements[t][0])):
                ax.annotate(i, (self.radarMeasurements[t][0][i], self.radarMeasurements[t][1][i]))
        if showTrueTrajectoriesMeasurements:
