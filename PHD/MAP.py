...
class MapGenerator:
    ...
    def addBorderBorningAreas(self):
        print("TODO")
        raise Exception("This method is not done yet.")
    ...
    def getTrajectoriesIDInTimeT(self, T):
        ...
        for i, traj in enumerate(self.trajectories):
            if traj.startTime<=T and traj.startTime+traj.ndat>T:
                trIDs.append(i)
        return trIDs
    def getAllTrueMeasurementsWithinRadarRadius(self):
        measurementsL2=[]
        for t in range(self.ndat):
            ...
            for coord in range(len(self.R)):
                ...
                for trID in actTrajectories:
                    print(self.trajectories[trID].Y[coord][t - self.trajectories[trID].startTime])
                    print("--------------")
    ...
    def addGlobalClutter(self, NSinglePoints=0, offset=0.001):
        if(len(self.R)>2):
        ...
        for t in range(self.ndat):
            ...
            Y[0, len(trueMeasurements[t][0]):] = np.random.uniform(low=X0min * (1 - np.sign(X0min)*offset), high=X0max * (1 + np.sign(X0max)*offset),
                                                                    size=clutterCount[t])
            Y[1, len(trueMeasurements[t][0]):] = np.random.uniform(low=X1min * (1 - np.sign(X1min)*offset), high=X1max * (1 + np.sign(X1max)*offset),
                                                                    size=clutterCount[t])
            self.allMeasurements.append(Y)
    ...
    def animateAll(self, showTrueTrajectories=True, showTrueTrajectoriesMeasurements=True, showRadar=True):
