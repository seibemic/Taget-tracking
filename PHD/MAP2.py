...
class Radar(MapGenerator):
    ...
    def animateRadar2(self,t,ax, showTrueTrajectories=True, showTrueTrajectoriesMeasurements=True, showRadar=True,
                         showMapClutter=True, showRadarClutter=True, showBorderAirports=True,
                         showAirports=True):
            ...
            if showTrueTrajectories:
                for i, traj in enumerate(self.trajectories):
                    ax.plot(traj.X[0], traj.X[1], "-", color=self.traj_colors[i % len(self.traj_colors)],label=f"True trajectory, T={traj.startTime}")
            if showMapClutter:
                ax.plot(self.allMeasurements[t][0], self.allMeasurements[t][1], "*", color="gold", label="Map Clutter")
            if showRadarClutter:
                ax.plot(self.radarMeasurements[t][0], self.radarMeasurements[t][1], "*", color="orange", label="Radar Clutter")
            if showTrueTrajectoriesMeasurements:
                ax.plot(self.trajectoriesMeasurements[t][0], self.trajectoriesMeasurements[t][1], "*r", label="Trajectory measurement")
            if showRadar:
                radar = plt.Circle((self.radarPosition), self.radarRadius, color="b", fill=False, label="Radar")
            ...
            if showAirports:
                for airPort in self.airports:
                    confidence_ellipse(airPort.pos[:2], airPort.cov, ax=ax, edgecolor="black", label="Airport")
...
#     r.setRadarPosition([100,100])
    ...
    # r.addNRandomAirports(2, airportCov,0.6)
    # r.addNBorderAirports(36, airportCov,0.4)
